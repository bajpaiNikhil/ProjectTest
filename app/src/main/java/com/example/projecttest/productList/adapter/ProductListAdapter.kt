...
class ProductListAdapter(private val productList : List<ProductItem> , private val roomViewModel : ItemProductViewModel) : RecyclerView.Adapter<ProductListAdapter.ItemViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        holder.binding.selectedCb.setOnClickListener { v ->
            if ((v as CheckBox).isChecked) {
                ...
                dispatch = false
                picked
                Log.d("MainActivity" , "Value if Check")
            }else{
                selectedCb = false
                Log.d("MainActivity" , "Value if UnCheck ")
            }
        }
